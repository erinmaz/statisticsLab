...
---
Had Tukey's been conducted as the *post-hoc* test, SPSS would have issued another table in the output file. This table is called “Homogenous Subsets.” (Please recall: Tukey’s is commonly used when homogeneity of variances is assumed. In this example, this is **NOT** the case.)

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/8.4.22.png')
```

This table actually gives you the same kind of information as the “Multiple Comparisons” table, but is more compact and easier to use. In this table you will see the conditions listed on the left, and a number of "subsets"--in this case there are 2 subsets (2 columns labeled 1 and 2, respectively). In these columns, the means of the conditions are listed (in this case, mean number of intrusive memories).
...
Using this lab’s data set, think about the following questions: 

1. What if participants’ intrusive memories were not actually affected by the manipulation (Tetris and Reactivation), but rather the group with the most intrusive memories just happened to react more strongly to the disturbing films? Are there differences in “level of disturbance” (Hint Hint: post-film distress rating) among the 4 groups? Test this hypothesis using an alpha level of .05. Report your result in proper statistical reporting format. What do you conclude? 

You will use the Moodle submission link to upload your .spv file showing you have completed the appropriate analysis.

You will use the Moodle quiz to show your interpretation of the results in a written Results sentence (or in written Results sentences if need be).
...
3. Are there differences among the `Number of Provocative Task Intrusions` (also known as “Intrusion Provocation Task [IPT]:  Number of image-based intrusive memories during 2-minute laboratory task on Day 7”) among the 4 groups? Test this hypothesis using an alpha level of .05. If the analysis shows a significant result, run follow-up tests using both approaches: planned comparisons and a *post-hoc* test. 
If you run the planned comparisons, imagine you want to compare the following chunks of variance:
a) Chunk 1 (Reactivation + Tetris) vs Chunk 2 (No-Task Control, Tetris Only, and Reactivation Only)
b) hunk 1 (Reactivation + Tetris) vs. Chunk 2 (No-Task Control as well as Reactivation Only)
c) Chunk 1 (Reactivation + Tetris) vs. Chunk 2 (No-Task Control)

If you run the unplanned comparisons, assume the assumption of heterogeneity of variance is upheld or met.

Report your results in proper statistical reporting format. What do you conclude? 

You will use the Moodle submission link to upload your .spv file showing you have completed the appropriate analysis (analyses).

You will use the Moodle quiz to show your interpretation of the results in a written Results sentence (or in written Results sentences if need be) and to answer questions based on what you have learned about follow-up tests.

4. Knowing that Levene’s test was non-significant for Question 3, conduct the ANOVA using the General Linear Model menu. Remember our question: Are there differences among the `Number of Provocative Task Intrusions` (also known as “Intrusion Provocation Task [IPT]:  Number of image-based intrusive memories during 2-minute laboratory task on Day 7”) among the 4 groups? 

Request the planned comparisons assuming the same chunks of variance indicated in Question 3.

Request the unplanned comparisons if the *F*-test is significant.
