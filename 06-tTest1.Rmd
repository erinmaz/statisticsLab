...
First, let's open the relevant data file. The data file has been simplified and is available on the lab Moodle page under "T-Tests." Click on "Mehr et al. Simplified Data File" to download it, note the filename and where you've downloaded it to, and then open it in JAMOVI. Your data should look like this:
...
Now, click <span style="color:blue">OK</span>. JAMOVI will produce several output tables.
...
**Important terminology note:** `Baseline_Proportion_Gaze_to_Singer` and `Test_Proportion_Gaze_to_Singer` are represented as two variables in JAMOVI, but they refer to two levels of a single independent variable. You can think of the independent variable as timepoint with two levels: baseline and test.
...
Click <span style="color:blue">OK</span>, and JAMOVI will produce several output tables. The tables are as follows:
...
If you are satisfied with the chart, click OK and it will appear in an JAMOVI output window:
...
Then click <span style="color:blue">OK</span>. JAMOVI will generate several tables in the output window. They should look like this:
...
In Section 2.4.1 of the lab manual, you learned how to request descriptive statistics. If you return to the output generated by JAMOVI when we ran the paired-samples *t*-test, you should notice that the first table depicted in the output provides the mean, the sample size, the standard deviation, and the standard error of the mean for each measurement - Baseline_Proportion_Gaze_to_Singer and Test_Proportion_Gaze_to_Singer. 
