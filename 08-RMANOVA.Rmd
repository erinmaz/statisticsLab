...
Challenge: On your own and using the data set to be used in our lab demonstration, check the other assumptions. Were these assumptions met or violated? How do you know? (**Be prepared to answer these questions at the beginning of lab.**)
...
From this table, we see that there is a significant effect of stimulus on reaction time because *p* < .0001. We might say it more formally as follows:

A one-way repeated measures ANOVA was conducted, and Greenhouse, and Greenhouse-Geisser estimates of sphericity were used to correct the degrees of freedom (ε = .59). The reaction times were significantly different among the types of stimulus, *F*(1.17, 43.46) = 420.33, *p* < .0001.

OR something like this:

A one-way repeated measures ANOVA was conducted with Greenhouse-Geisser estimates of sphericity were used because Mauchly’s test indicated a violation to the assumption, *χ^2* = 0.30, *p* > .05. The results indicated that reaction times were significantly different among the types of stimulus, *F*(1.17, 43.46) = 420.33, *p* < .0001.
...
If you had *a priori* hypotheses about where the difference(s) may be, you would conduct planned comparisons. In JAMOVI, you would do this by conducting paired-samples *t*-tests because, in each comparison, the reactions times are being measured from the same people. Remember to apply a Bonferroni correction (Divide your alpha by the number of *t*-tests planned.).
...
We could go through the menus again to request the *post-hoc* tests, or we could simplify the output in the Results pane by clicking on the Results to have the commands pop up to the left. Let’s do the latter. Next, w scroll down to the “Post Hoc Tests” ribbon, move the factor (`Typing_Material_Condition`) to the window on the right, and then select the correction you want applied. Let’s select <span style="color:blue">Bonferroni</span>.
