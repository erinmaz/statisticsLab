...
Move your variables of interest, T1_sleep_quality_composite and T1_anxiety, to the right box:
...
The table indicates that Pearson's *r* between the two variables is -0.425, with a *p*-value of less than .001. You could write that result in APA format as follows:

>There was significant correlation between sleep quality and anxiety at the beginning of midterms, Pearson's *r* = .43, *p* < .001.
...
To accompany the calculation of the correlation coefficient, the scatterplot is the relevant visualization tool. Depending on your version of JAMOVI, you may have the option to enable a correlation matrix plot from the correlation matrix analysis panel:
...
Before you can request a larger scatterplot in jamovi, you must add Scatterplot as a module. To do so, click on the addition sign which is white with a blue trim that has the word “Modules” under the sign. Click <span style="color:blue">jamovi library</span>. Under the “Available” tab, you should see a module called <span style="color:blue">scatr</span>. Click to <span style="color:blue">INSTALL</span> it. When it is installed, it will appear in your <span style="color:blue">Exploration</span> menu.
