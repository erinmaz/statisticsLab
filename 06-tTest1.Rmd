...
In Section 2.5.1 of the lab manual, you learned how to request descriptive statistics. If you return to the output generated by jamovi when we ran the one-samples *t*-test, you should notice that we went back to request this Descriptives table in the output to provide the mean, the sample size, the standard deviation for each measurement - Baseline_Proportion_Gaze_to_Singer and Test_Proportion_Gaze_to_Singer. Do note, however, that you do not have to make this request for the Descriptives table in a second analysis.
...
A window will appear. Using highlighting and the arrow, move the variable of interest, `Difference_in_Proportion_Looking`, into the "Dependent Variables" field. Below that, make sure to enter 0 in the <span style="color:blue">Test value</span> field which is under the bolded section called <span style="color:blue">Hypothesis </span>. 
