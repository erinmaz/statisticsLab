...
1. Select and calculate descriptive statistics
2. Create and interpret a boxplot

### Pre-lab tasks

---

<b>Please complete all of these tasks before your lab starts.</b> Do these tasks as soon as possible and post to the Q&A forum or come to virtual help hours if you run into any trouble. 

If you have not already done so, read the "Expectations for lab" posts in our Q&A forum: <a href="https://moodle.stfx.ca/mod/forum/view.php?id=815450" target="_blank">https://moodle.stfx.ca/mod/forum/view.php?id=815450</a>.

We have updated the EngageNS dataset. Please delete your current version and download the latest csv file here: <a href="https://moodle.stfx.ca/mod/url/view.php?id=830017" target="_blank">https://moodle.stfx.ca/mod/url/view.php?id=830017</a>. If this link does not work for you, you may need to complete the EngageNS terms of use quiz here: <a href="https://moodle.stfx.ca/mod/quiz/view.php?id=830016" target="_blank">https://moodle.stfx.ca/mod/quiz/view.php?id=830016</a>.
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/import_browse.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/deletecol.png')
```
...
You are all ready for lab! You can close everything for now, but before your lab session begins, open your .omv file in JAMOVI and your data dictionary pdf in a pdf viewer such as Preview or Adobe.

<b>Come to your lab session with your .omv file open in JAMOVI and your data dictionary pdf open in a pdf viewer such as Preview or Adobe.</b>
...
Suppose we wanted to know about Nova Scotian’s sleep duration. We can use both descriptive statistics and graphs such as histograms to explore a variable such as sleep duration. 
...
Let's look at another variable. The `VOLUNTEER` variable represents whether participants volunteered in the past year. A 1 means they did, and a 2 means they didn't. Take a moment and make a prediction: What percentage of participants do you expect have volunteered in the past year?
...
Click the Analyses tab, then click Exploration and then Descriptives. Search for VOLUNTEER and move the variable into the Variables box (note: there's no `R_VOLUNTEER` variable so we don't have to worry about that in this case). As before, select all the measures of Central Tendency and Dispersion. Your table should look like this:
...
Thinking about scales of measurement, this is an example of an ordinal variable. Let's look at the `BELONGING` variable in the dataset. Select the variable (remember you can search in the Variable tab first to find it faster) and click Setup.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/belongingsetup.png')
```

This time, we need to change the measurement type. JAMOVI thinks this is a nominal variable, but we know it is ordinal. Change it in the dropdown menu. You may get an error and it may not appear to work. Check whether it worked by closing the Setup panel by clicking the up arrow. You should notice that the icon next to `BELONGING` in the spreadsheet now corresponds to an ordinal variable, so we know we successfully set the measurement type to ordinal.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/belongingordinal.png')
```

Let's look at how Nova Scotia's rated their sense of belonging. As before, click on the Analysis tab, then Exploration, then Descriptives. Search for the `BELONGING` variable and move it into the Variable box. This time, we are only going to select the descriptive statistics that are appropriate. BELONGING is an ordinal scale of measurement, so we are going to use the median as a measure of central tendency, and the IQR as a measure of variation. Your JAMOVI window will look like this:
...
Using the `SLEEP` and `R_SLEEP` variables, let's create boxplots. Move both variables into the Variables box. Under Plots, select Box plot. In the Results panel, jamovi will produce output containing two boxplots.
...
Because .omv files contain both the data and the output, we want to save a new file before each analysis. So, before you start your homework, save a new copy of your .omv file by clicking on the three lines and selecting Save As. Give the file a new name. Then, as you generate output by following the instructions below, you can continue to save this file.

To save just your output, click the three lines and select Export. This will allow you to save a pdf. 
...
Use the data dictionary to determine the scale of measurement of the following variables that relate to internet access.

Calculate an appropriate measure of central tendency for each variable.
Calculate an appropriate measure of variation for each variable.

a. Internet access of Nova Scotians using any 2 of the following 3 variables: `INTERNET`, `NET_QUAL`,  and `HM_SURF`.
