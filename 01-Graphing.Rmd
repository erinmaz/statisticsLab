...
1. Opening jamovi and reviewing the jamovi layout 
2. Reviewing variable properties and the Setup button 
3. Opening a data file and producing different types of graphs 
...
Your lab instructor will take you through the process of downloading, installing, and opening the jamovi program. You may double-click on its icon located on the desktop of your computer, or you may find it using the Start menu. Once the program loads, you can click on the button that looks like three horizontal lines at the top left. This bottom reveals a menu. Take note that you have some familiar options here: New (for a new file), Open (to open an existing file), and Save. For now, we will be examining the basic layout of jamovi without a data set, so you can click the left facing arrow to close this menu.

Once you do, the main jamovi spreadsheet should be apparent. It will look like this, with a basic spreadsheet on the left and a results pane on the right:
...
### Reviewing variable properties and the Setup button

---

Now that we have some data entered, we might want to name our variable so that it’s evident our measurements represent heights. In order to view or modify variable names and other properties, look to the menu ribbon at the top of the jamovi window. While you are within any column representing a variable, you can click the <span style="color:blue">Setup</span> button which is visible when the <span style="color:blue">Data</span> tab is highlighted. This <span style="color:blue">Setup</span> button will reveal, in a top panel, information about the attributes of the variable. 
...
Now that we know about the properties of the jamovi spreadsheet window, let’s open a data file and learn how to make some sense of it by creating different types of graphs. (It is not important for you to <span style="color:blue">Save</span> the jamovi file you just created because we will not be doing anything with those fictitious height data.) Click the button that looks like three horizontal lines at the top left. Select <span style="color:blue">Open</span> and let’s open the EngageNS data set which was made available to you via a link. Remember that EngageNS surveyed Nova Scotians about various aspects of their quality of life.

The data dictionary was also provided to you via a link. The dictionary lists all the variables that were included in the EngageNS survey and shows how they were measured.

Once you open the data file, browse through to familiarize yourself with the variables that are being measured. Switch over to Variable View for details of each variable. Take a look at the rows and columns in each file. Each *column* represents a *different variable* that was measured in the EngageNS survey. Each *row* represents the responses of a *single survey respondent*.
...
Now, let's refocus on the spreadsheet view. We will not be working with every single variable in this spreadsheet, but we'll select a few interesting ones with which to answer questions. Let's start with `PARSPORT`. Suppose we wanted to know how many respondents in the EngageNS survey are members of or participants in sports or recreational organizations. Let's use jamovi to produce a graph to answer this question. With your data file open, go up to the `Analyses` tab on the top menu and choose <span style="color:blue">Exploration</span>, then <span style="color:blue">Descriptive</span>. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.1.png')
```

You will see an entire new window with the analysis options and results will appear in the Results pane as you select options for the analysis. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.2.png')
```

The window that appears contains every variable in your spreadsheet listed on the left-hand side. We can choose one variable at a time to plot. Notice that jamovi allows you to search through that long list of variables by clicking on the magnifying glass icon. It will be important to know the names of the variables for which you are searching. Look in the data dictionary to find the names of variables of interest to you as you run your analyses.

Let's think about the nature of our question. We would like to know how many people in the data set indicated that they participate in sports and/or recreational organization. `PARSPORT` is the name of the variable in jamovi, and it is measured using a Yes/No response scale. This is a nominal scale variable and so we will appropriately choose a BAR graph to plot it. 

In the Descriptives window, select the variable of interest for the graph by highlighting `PARSPORT` and using the arrow to move it to the Variables window on the right. Then, click on <span style="color:blue">Plots</span>. You will see a number of options available. Select  <span style="color:blue">Bar plot</span>…

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.3.png')
```

In the Results pane, you will see a Descriptives table was produced when you selected the variable, and a bar graph was produced when you selected <span style="color:blue">Bar plot</span>.  Notice how many respondents are involved in sports/recreational organizations and how many are not. Roughly how many people participate in sports/recreational organizations? How many do not?

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.4.png')
```

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.5.png')
```

To answer the questions about participants involved in sports/recreational organizations, we need to turn to the data dictionary to find out what the codes 1 and 2 represent. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.6.png')
```

When viewing the data in `PARSPORT`, we read the code 1 as YES and the code 2 as NO. By looking at the bar graph, we might estimate that about 3000-3500 respondents participate in sports/recreational organizations and perhaps about 9000 do not.

Our purpose in this section is to make graphs and read them for information; however, let's take the chance to discuss a way you could quickly check your estimates. In the Descriptives window, when requesting the descriptive table and bar graph, we could also select the variable click on <span style="color:blue">Frequency</span>. You will see a frequency table appear in the Results pane. The reported frequencies are in line with our estimates based on the bar graph. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.7.png')
```

Your bar graph will have as many bars as there are categories in your nominal variable. For example, if you want to know how many respondents were from each region of NS, create a bar graph for REGION. The number of regions from which the survey participants come is represented by the number of bars on your graph.

If you want to construct this graph without moving both `PARSPORT` and `REGION` into the same Descriptives table, you can click on the arrow that faces to the right to close the Descriptives analysis and bar plot that you just completed, and then, click on the <span style="color:blue">Exploration</span>  and <span style="color:blue">Descriptives</span>  commands again. Now, you can repeat the process looking at only `REGION`. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.8.png')
```

From how many regions in Canada did the survey participants come?

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.9.png')
```
...
Here, we will deal with people's self-reported mental health recorded as variable `MNTLHLTH`. In the EngageNS survey, respondents were asked to rate their own mental health on a 5-point scale, ranging from 1 to 5 where 1 = *poor*, 2 = *fair*, 3 = *good*, 4 = *very good*, and 5 = *excellent*.

TO BE DEVELOPED: DISCUSSION ABOUT ERRORS IN jamovi’s READING OF THE VARIABLE ATTRIBUTES. IT IS NOT A NOMINAL VARIABLE. IT MIGHT BE CONSIDERED ORDINAL OR INTERVAL. ALSO, THIS IS AN APPROPRIATE PLACE TO SHOW STUDENTS HOW TO ADD THE LEVELS/VALUE LABELS BY GOING BACK TO THE DATA DICTIONARY TO THINK ABOUT WHAT THE 1-5 MEAN.
Visual for errors in variable attributes:
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.9B.png')
```
Visual for codes and what they represent from data dictionary:
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.9C.png')
```

Let's plot a histogram for this variable. (Remember to close the Descriptives menu, by clicking on the arrow facing to the right, before you keep going unless you want to run the same analyses on a number of variables.) We want to put our variable of interest - `MNTLHLTH` - on the x-axis and frequency for each score on the y-axis.  If you will recall, there were a number of options for types of graphs in the sub-menu we used for the bar graph. It is in that sub-menu where we will find the histogram option. 

Under the <span style="color:blue">Analyses</span>  tab, choose <span style="color:blue">Descriptives</span> in the top menu. Let’s either use the search function or scroll through the variable list to find `MNTLHLTH`. Using the arrow, move that variable into the "Variables" window. Now, click on <span style="color:blue">Plots</span>. Remember we want a histogram, not a bar graph, so let’s select <span style="color:blue">Histogram</span>…

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.10.png')
```

jamovi will produce an output window containing the distribution of scores for respondents' self-reported mental health. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.11.png')
```

The histogram produced is not helpful in terms of allowing us to view numbers of respondents on the *y*-axis. Let's click to select <span style="color:blue">Frequency tables</span> so we can view the numbers of respondents under each code. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.12.png')
```

Using the <span style="color:blue">Save as</span> command found when you click on the button that looks like three horizontal lines in the menu at the top of the jamovi screen, remember to save and name your file if you think you may want to look at it again in the future.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.5.3.13.png')
```
...
Create the following two graphs in a single jamovi file (*.omv). Upload this file to Moodle and use the graphs to answer the questions in the homework “quiz” on Moodle. Note that you can attempt the quiz as many times as you like before the due date and only your highest mark will be counted.

Use the EngageNS data to answer the following questions and to create graphs:

1. Respondents were asked to indicate their main form of transportation to get to their main job.  The variable name is WORK_TRANS.  Responses were given on the following response scale: 1 = walk, 2 = bicycle, 3 = personal car, 4 = public transport, 5 = other. 

a. Create the most appropriate type of graph for this data. 

b. Which form of transportation was used by the most respondents?  The least?

2. Respondents were asked to rate the overall accessibility of the health care services in the community. The variable name is ACCESS_HLTH. Responses were measured on a 5-point scale: 1 = poor, 2 = fair, 3 = good, 4 = very good, 5 = excellent.

a. Create the most appropriate type of graph for this data. 

b. What do you learn by looking at the graph?  

c. What would you suggest to government officials about access to health care based on this data?  
