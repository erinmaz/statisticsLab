...
## JAMOVI - Week 1 - January 10th & 11th 
...
#### Computed Variables - Week 4 - January 31st & February 1st
...
Imagine we have a participant who scores in the following way:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ScoresOnPSS.png')
```

Enter these scores into a JAMOVI data file of your own (creating a new file), or go to [Moodle](https://moodle.stfx.ca/course/view.php?id=29319#section-9) to download and open the data file named "Fictitious Data Set - PSS_RecodingAndDataComputations_OneParticipant." Then, follow along with the proceeding commands to be sure you feel comfortable using JAMOVI to recode data and compute new variables.
The score on the PSS is calculated by reversing the answers to the questions that are oppositely worded (questions 4, 5, 7, and 8) and, then, summing the scores.

Let’s work through this process with the participant’s scores. We can begin by recoding item 4, shown as `PSS4` in the data file. This process is similar to, but not the same as, computing a new variable. While the column for that item is highlighted, select <span style="color:blue">Data</span>, and then, select <span style="color:blue">Transform</span>.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4.png')
```

A new column will appear to the right of the column for the `PSS4` and the variable attributes panel will drop down. (As you can imagine, you could also use these commands: <span style="color:blue">Data</span>, <span style="color:blue">Add</span>, and under "Transformed Variable," either <span style="color:blue">Insert</span> or <span style="color:blue">Append</span>.) You can fill in the information for what you want this variable to be called and record its notable attributes. For our purposes, let’s call it `PSS4_Recoded`. You can see that the source variable is correctly shown as `PSS4`. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_2.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_3.png')
```

When you do click that button, a new panel will appear. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_4.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_5.png')
```

A new panel opens, and we will enter the formula to have the transformations calculated. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_6.png')
```

Because 4 is the maximum score value and 0 is the minimum score value, we can write this equation to get the recoded values:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_7.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_8.png')
```

To get out of  the Transformed Variable panel, click on the upward facing arrow.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_9.png')
```

You should see the recoded PSS4 value in the `PSS_Recoded` column.

Repeat the steps to recode `PSS5`, `PSS7`, and `PSS8`. For these cases, you will not need to select <span style="color:blue">Create New Transform</span>. Instead, you will select the transformation rule you already created, <span style="color:blue">Reverse_Coding </span>.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_10.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_11.png')
```
...
Move to the column to the right of `PSS10`, and select <span style="color:blue">Data</span> and select <span style="color:blue">Compute</span>.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_12.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_13.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_14.png')
```
...
This may seem like a lot of work to do for one participant’s sum; however, these commands and having JAMOVI do this recoding and computation work is a mega timesaver when you are working with a large data set compiled from the scores given by a larger sample.
...
Looking at the following fictitious data showing the `PSS_Score` for 10 participants, let’s look at the process to get the *z*-scores for those `PSS_Score` values. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/RecodingScoresOnPSS4_15.png')
```

To follow along with the proceeding commands, you might add the other `PSS_Score` values in that column (if you created your own data file), or you can go to  [Moodle](https://moodle.stfx.ca/course/view.php?id=29319#section-9) to download the data file called "Fictitious Data Set - PSS_RecodingAndDataComputations_TenParticipants_RecodedAndComputed," and then, in JAMOVI, open that data file. 

In JAMOVI, *z*-scores can be obtained by selecting <span style="color:blue">Data</span> and <span style="color:blue">Compute</span>. As before, you can click on <span style="color:blue">Setup</span> while the column for the new variable is highlighted to fill in the Computed Variable attributes. When entering those attributes, you should also enter the formula for the *z*-score. The Z is found in the formula editor’s drop-down menu, but it can also be typed in. The variable name is in that drop-down menu, too. Similarly, it can be type instead.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ZScoresAsComputedVariables.png')
```

Remember to click the upward facing arrow to close the Computed Variable panel. `Z_PSS_Score` in the image below depicts the *z*-scores for `PSS_Score`. Notice that computed variables have a black circle to the right of their name near the top of the column.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ZScoresAsComputedVariables_1.png')
```

##### Homework

See [Moodle]( https://moodle.stfx.ca/course/view.php?id=29319) for homework questions.
