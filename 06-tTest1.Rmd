...
For this lab, we will be using the data from Mehr, Song, and Spelke's (2016) study of how music may convey important social information to infants. You can access that [data set](https://moodle.stfx.ca/mod/resource/view.php?id=815524) here.
...
First, let's open the relevant data file. The data file has been simplified and is available [here](https://moodle.stfx.ca/mod/resource/view.php?id=815524). Right click on "Mehr et al. Simplified Data File" and indicate that you want it opened it in jamovi. Your data should look like this:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.35.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.36.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.37.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.38.png')
```

Now, if you look to the Results panel (which is on the right-hand side), you should see a table indicating the results of the one-sample *t*-test.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.39.png')
```
...
Typically, however, we would include the descriptive statistics in this write-up as well. You might remember the skills you learned in Lab 2, Section 2.5.1, about requesting the descriptive statistics for this variable (Click on <span style="color:blue">Analyses</span>, <span style="color:blue">Exploration</span>, and then <span style="color:blue">Descriptives</span>, move the variable of interest to the Variables window, and select the desired statistics that are appropriate based on the scale of measurement, and look to the Results panel for the Descriptives table.).

Alternatively, **and much more conveniently**, you could click on the One Sample T-test table in the Result panel. This will bring up the Analyses panel again. In here, you can click to add <span style="color:blue">Descriptives</span>:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.40.png')
```

The Results panel will automatically be updated to include in that analysis the newly requested information. The first table - the one with the one-sample *t*-test results - is still visible; however, there should now be a second table - a Descriptives table. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.41.png')
```

We can see from this latter table that the average proportion of time spent looking at the person who would later be the familiar song singer is .52. This is very close to .50. 
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.42.png')
```

A window will appear with a list of all variables in a field on the left. We must choose the two variables that contain our data: `Baseline_Proportion_Gaze_to_Singer` and `Test_Proportion_Gaze_to_Singer`. Move these into the right-hand field, labelled “Paired Variables,” using the arrow. You can move one level of the variable at a time, or you can highlight both by using the Control key and then move them at the same time. Make sure that they appear next to each other in the right-hand field, as follows:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.43.png')
```

Similar to what you just learned about requesting the descriptive statistics in the One Sample T-Test command, you can request these descriptive statistics through the same type of commands in the <span style="color:blue">Paired Samples T-Test</span> command options. That is to say, jamovi offers the option to produce the Descriptives table for the two levels of the variable within the pop-up window for the *t*-tests. Click to select the <span style="color:blue">Descriptives</span>. You may also want to select the <span style="color:blue">Mean difference</span> if you plan to report it in your Results. Alternatively, you will report the means and standard deviations for the two levels of the variable. You may also be interested in reporting the <span style="color:blue">Effect size</span> and the 95% <span style="color:blue">Confidence interval</span>.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.44.png')
```

In the Results panel, you should see two tables. The first table shows the results of the paired-samples *t*-test. The table indicates a *t*-value (i.e., our observed *t*), degrees of freedom (*N*-1), and a *p*-value. You can interpret these values the same way as in the previous example. The second table is the Descriptives table. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.45.png')
```

We might report our results in this way:

During the baseline condition, the difference between infants' mean proportion looking time toward the singer at baseline and the test phase was -0.07, and this difference was significant according to a paired-samples *t*-test, *t*(31) = -2.42, *p* < .05, 95% CI [-0.79, -0.06], *d* = -0.43.

### Using descriptive statistics to give context to inferential statistics

The descriptive statistics are useful for contextualizing the inferential statistics. In this case, we see that, for `Baseline_Proportion_Gaze_to_Singer`, the mean is 0.52 while for `Test_Proportion_Gaze_to_Singer`, the mean for 0.59. The standard deviation for both rounds to 0.18. In Section 6.7.3, we reported the mean difference of -0.07 was significant according to the paired-samples *t*-test. Now, we might add another sentence to explain the difference. 

Using the means and the standard deviations for each level of the independent variable, we will generate a more detailed interpretation which includes a statement about whether there is an increase or a decrease:

According to a paired-samples *t*-test, there is a significant difference between infants' mean proportion looking time toward the singer at baseline (*M* = 0.52, *SD* = 0.18) and the test phase (*M* = -0.59, *SD* = 0.18), *t*(31) = -2.42, *p* = .02, 95% CI [-0.79, -0.06], *d* = -0.43. 

### Graphing your results

---

In order to visualize the difference between means (for a paired-samples *t*-test, in this case), it is most appropriate to create a bar graph, or bar plot. Because our two conditions (baseline and test) make up a discrete (and not continuous) categorical variable (also called a nominal variable), a bar graph is most appropriate. Unfortunately, it is not so easy to produce a bar graph with jamovi. In this example, it might be easy enough, but graphing would be increasingly cumbersome with an increasing number of participants or cases and an increasing number of levels of the variable. Your lab instructor will guide you through a work-around solution.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.46.png')
```
...
Thus, the paired-samples *t*-test is exactly the same as the one-sample *t*-test if we consider it as a comparison of the mean difference score to 0. Let's demonstrate this using jamovi. 

We'll continue using the same data as above. Specifically, locate the variable named `Difference_in_Proportion_Looking`. This variable contains the difference score between the baseline and test conditions for each infant. We're going to treat this as our one-sample (as in the one-sample *t*-test), and we'll be comparing it to a mean of 0. 

Let's begin. Go to <span style="color:blue">Analyses</span>, then <span style="color:blue">T-Tests</span>, and then <span style="color:blue">One Sample T-test</span>.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.47.png')
```

A window will appear. Using highlighting and the arrow, move the variable of interest, `Difference_in_Proportion_Looking`, into the "Dependent Variables" field. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.48.png')
```

Below that, make sure to enter 0 in the <span style="color:blue">Test value</span> field which is under the section with the bolded title "Hypothesis". 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.49.png')
```

The Results panel in jamovi should look like this:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/6.4.50.png')
```

You will see that the *t*-statistic and *p*-value are identical to the ones obtained when we conducted a paired-samples *t*-test. Therefore, a paired-samples *t*-test conducted on two samples is equivalent to a one-sample *t*-test conducted on the difference scores (calculated on the two samples) compared against 0. 

Nota Bene: You will see that in the previous paired-samples *t*-test we obtained a *t*-statistic of -2.42, but here the *t*-statistic is positive. This difference comes from the fact that jamovi automatically subtracts the later-occurring variable in your spreadsheet from the earlier-occurring one. If your variables were ordered differently, you would obtain a positive *t*-statistic as well. In other words, the sign does not matter; it results from the order of the variables. 
...
1. On average, adults should get roughly 8 hours of sleep per night.  Using the data collected in PSYC 291, do students get enough sleep during midterm week?  That is, use the average number of hours of sleep students get during the week prior to the Time 2 data collection.  Conduct the appropriate *t*-test.  What can you conclude?  Write your answer in APA format, including the 95% confidence interval, and effect size (Cohen’s *d*).

2. Does the quality of students’ sleep change between the time early in the academic year (i.e., Time 1 – September) and midterms (i.e., Time 2 – mid-October)?  Using the data collected in PSYC 291, calculate the composite for students’ perceptions of the quality of their sleep at both Time 1 and Time 2.  Conduct the appropriate *t*-test.  What can you conclude?  Write your answer in APA format, including the 95% confidence interval, and effect size (Cohen’s *d*).  Using APA format, graph the data.  
...
2. Using the data collected in PSYC 291, are students more or less anxious than “somewhat anxious” (i.e., 2 on the scale) during midterm week? Conduct the appropriate *t*-test.  What can you conclude?  Write your answer in APA format, including the 95% confidence interval, and effect size (Cohen’s *d*).

Repeated Measures *t*-tests

1. Does student stress level change between the time early in the academic year (i.e., Time 1 – September) and midterms (i.e., Time 2 – mid-October)?  Using the data collected in PSYC 291, calculate the composite for students’ perceptions of stress at both Time 1 and Time 2.  Conduct the appropriate *t*-test.  What can you conclude?  Write your answer in APA format, including the 95% confidence interval, and effect size (Cohen’s *d*). Using APA format, graph the data.  

2. Do students engage is less physical self-care during midterms (i.e., Time 2) than they do early in the academic term (i.e., Time 1)?  Using the data collected in PSYC 291, calculate the composite for the physical self-care subscale at both Time 1 and Time 2 of the Mindful Self-Care Measure.  Conduct the appropriate *t*-test.  What can you conclude?  Write your answer in APA format, including the 95% confidence interval, and effect size (Cohen’s *d*). Using APA format, graph the data.  

3. Does substance abuse change between Time 1 (early September) and Time 2 (mid-October)?  Using the data collected in PSYC 291, conduct the appropriate *t*-tests for both alcohol consumption over the last week and cannabis consumption over the last week.  (Note that this requires 2 separate *t*-tests – one for alcohol and one for cannabis).  What can you conclude about each substance?  Write your answers in APA format, including the 95% confidence intervals, and effect sizes (Cohen’s *d*). Using APA format, graph the data (note that you will need to make up one graph for each substance).  
