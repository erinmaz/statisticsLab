...
This section is  copied almost verbatim, with some editorial changes, from [Answering questions with data: The lab manual for R, Excel, SPSS and JAMOVI, Lab 1, Section 1.4, SPSS](https://www.erinmazerolle.com/statisticsLab/), according to its [CC license](https://creativecommons.org/licenses/by-sa/4.0/deed.ast). Thank you to Crump, Krishnan, Volz, &   Chavarga (2018). 
</div>
...
2. Reviewing variable properties and the Settings button 
3. Opening a data file and producing different types of graphs 
...
You may want to click the arrowhead that points to the left or right to move to an adjacent variable. Alternatively, clicking the <span style="color:blue">Settings</span> button again or clicking the upward facing arrow will hide this variable attribute information from your view.
...
INSERT SCREENSHOT FOR PARSPORT PLOT
...
Here, we will deal with people's self-reported mental health. In the EngageNS survey, respondents were asked to rate their own mental health on a 5-point scale, ranging from 1 to 5 where 1 = *poor*, 2 = *fair*, 3 = *good*, 4 = *very good*, and 5 = *excellent*.
...
Let's plot a histogram for this variable. (Remember to close the Descriptives menu, by clicking on the arrow facing to the right, before you keep going unless you want to run the same analyses on a number of variables.) We want to put our variable of interest - mental health - on the x-axis and frequency for each score on the y-axis.  If you will recall, there were a number of options for types of graphs in the sub-menu we used for the bar graph. It is in that sub-menu where we will find the histogram option. 

Under the <span style="color:blue">Analyses</span>  tab, choose <span style="color:blue">Descriptives</span>  in the top menu. Let’s either use the search function or scroll through the variable list to find `MNTLHLTH`. This is the variable containing respondents’ ratings of their mental health. Using the arrow, move that variable into the "Variables" window. Now, click on <span style="color:blue">Plots</span>. Remember we want a histogram, not a bar graph, so let’s select <span style="color:blue">Histogram</span>…

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.4.19.png')
```

jamovi will produce an output window containing the distribution of scores for respondents' self-reported mental health. (Using the <span style="color:blue">Save as</span> command found when you click on the button that looks like three horizontal lines in the menu at the top of the jamovi screen, remember to save and name your file if you think you may want to look at it again in the future.) 
